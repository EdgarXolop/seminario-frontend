<template>
  <section class="section">
    ...
    <b-table
        ...
        <template #empty>
            <div class="has-text-centered">No hay alumnos registrados.</div>
        </template>
...
const {
  ...
  VUE_APP_ID_USUARIO_TIPO_ALUMNO
  }  = process.env;
...
export default {
  ...
  data() {
      ...
      alumno: {
        ...
        telefono: '',
        email: '',
        tipo: VUE_APP_ID_USUARIO_TIPO_ALUMNO,
        domicilio: '',
      }
    }
  },
  methods: {
    submit(props){
      ...
      if ( this.alumno.id != 0 )
        request = this.axios.put(VUE_APP_API_BASE_URL + API_PATH, this.alumno, {params:{id:this.alumno.id}})
      else
        request = this.axios.post(VUE_APP_API_BASE_URL + API_PATH, this.alumno)
    ...
    loadData(){
      this.isLoading = true;
      this.axios.get(VUE_APP_API_BASE_URL + API_PATH, {params:{tipoUsuario:VUE_APP_ID_USUARIO_TIPO_ALUMNO}})
      .then((response) => {
